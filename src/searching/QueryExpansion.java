...
public class QueryExpansion {
    ...
    public static double spud_mu = 0;
    ...
    public Map<String,Double> expansionTerms(IndexReader reader,CustomQuery query, int query_length) throws IOException{
        ...
        }else if (QueryExpansion.method == QueryExpansion.TA){
            
            //get SMM estimates
             
            expansion_terms = this.TA(query, reader);
                ...
                if (map != null) {
                    ...
                    for (String term : map.keySet()) {
                        ...
                        if (!term.contains(":")) {
                            ...
                            } else if (QueryExpansion.method == QueryExpansion.SPUDQTM2) {
                                //SPUD Topic Model
                                prob = (((double) ((1 - spud_pi) * tf / this.pdoc_lengths[i]) / (((1 - spud_pi) * tf / this.pdoc_lengths[i]) + spud_pi * (df / sum_df))));
                                e =  (0.5*spud_mu + cf*prob )* Math.exp(this.pdoc_scores[i]) / (spud_mu + cf);
        ...
        for (int i=0;(i<num_expansion_terms)&&(i<list.size());i++){
           ...
           //logger.info(term + "\t" + topic_prob); 

           
           //System.out.println(term + "\t" + topic_prob + "\t" +  (double)((Map.Entry)list.get(i)).getValue());
           //logger.info(term + "\t" + topic_prob + "\t" +  (double)((Map.Entry)list.get(i)).getValue() + "\t" + df);
        }
    ...
    /**
     * This is not an expansion method.
     * This only reweights query terms according to 
     * Paik et al. 
     * 
     *  A Fixed-Point Method for Weighting Terms
     *  in Verbose Informational Queries
     * 
     * @param reader
     * @param query
     * @param e_terms
    ...
    private Map<String,Double> TA(CustomQuery query, IndexReader reader) throws IOException{
        
        double[][] array = new double[query.terms().size()][query.terms().size()];
        
        
        
        int k = 0, j = 0;
        for (String t1 : query.terms()) {
            j = 0;
            for (String t2 : query.terms()) {
                if (!t1.equals(t2)) {
                    Double cumRF = 0.0, tf1, tf2;
                    for (int i = 0; i < this.pdoc_lengths.length; i++) {
                        tf1 = this.pdocs[i].get(t1);
                        tf2 = this.pdocs[i].get(t2);

                        if (tf2 == null) {
                            if (tf1 != null) {
                                cumRF += Math.log(1 + tf1) / Math.log(2);
                            }
                        } else {
                            if (tf1 != null) {
                                cumRF += (Math.log(1 + tf1) / Math.log(2)) / (Math.log(1 + tf2) / Math.log(2));
                                //logger.info(t1 + "\t" + t2 + "\t" + Math.log(1 + tf1) + "\t" + Math.log(1 + tf2));
                            }
                        }
                    }
                    
                    //logger.info(j + "\t" + k + "\t" + cumRF + "\t" + t1 + "\t" + t2);
                    array[j][k] = cumRF;
                }
                j++;
            }
        ...
        //this does the power iteration
        Matrix x = new Matrix(query.terms().size(), 1, 1.0 / query.terms().size()); 
        for (int i = 0; i < 20; i++) {
            x = A.times(x);
        ...
        TreeMap<String, Double> qweights = new TreeMap();
        
        j=0;
        for (String t1:query.terms()){

            //logger.info(t1 + "\t" + x.get(j, 0) + "\t" + query.get(t1)/query.mass());
            Integer df = reader.docFreq(new Term("text",t1));
            double idf;
            if ((df == null)||(df == 0)){
                idf = 0;
            }else{
                idf = Math.log(reader.numDocs()/reader.docFreq(new Term("text",t1)));
            }
            qweights.put(t1, x.get(j, 0)* (idf/(10+idf)));
            //qweights.put(t1, x.get(j, 0));
